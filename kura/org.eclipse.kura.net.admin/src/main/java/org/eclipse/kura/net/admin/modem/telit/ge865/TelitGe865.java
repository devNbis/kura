...
/**
 * Defines the serial Telit GE865 modem
 * @author devNbis
 */
public class TelitGe865 extends TelitModem implements HspaCellularModem {

	private static final Logger s_logger = LoggerFactory.getLogger(TelitGe865.class);

	private int m_pdpContext = 1;

    /**
     * TelitGe865 modem constructor
     *
     * @param device - modem device as {@link ModemDevice}
     ...
     */
	public TelitGe865(ModemDevice device, String platform,
    ...
    public boolean isSimCardReady() throws KuraException {
    	...
    	synchronized (s_atLock) {
    		s_logger.debug("sendCommand getSimStatus :: " + TelitGe865AtCommands.getSimStatus.getCommand());
	    	byte[] reply = null;
	    	CommConnection commAtConnection = openSerialPort(getAtPort());
	    	if (!isAtReachable(commAtConnection)) {
	    		closeSerialPort(commAtConnection);
	    		throw new KuraException(KuraErrorCode.NOT_CONNECTED, "Modem not available for AT commands: " + TelitGe865.class.getName());
	    	}
			try {
				reply = commAtConnection.sendCommand(TelitGe865AtCommands.getSimStatus.getCommand().getBytes(), 1000, 100);
			} catch (IOException e) {
				closeSerialPort(commAtConnection);
	        ...
	        if (!simReady) {
	        	try {
					reply = commAtConnection.sendCommand(TelitGe865AtCommands.simulateSimNotInserted.getCommand().getBytes(), 1000, 100);
					if (reply != null) {
						sleep(5000);
						reply = commAtConnection.sendCommand(TelitGe865AtCommands.simulateSimInserted.getCommand().getBytes(), 1000, 100);
						if (reply != null) {
							sleep(1000);
							reply = commAtConnection.sendCommand(TelitGe865AtCommands.getSimStatus.getCommand().getBytes(), 1000, 100);
    ...
    public ModemRegistrationStatus getRegistrationStatus() throws KuraException {
    	...
    	synchronized (s_atLock) {
    		s_logger.debug("sendCommand getRegistrationStatus :: " + TelitGe865AtCommands.getRegistrationStatus.getCommand());
	    	byte[] reply = null;
	    	CommConnection commAtConnection = openSerialPort(getAtPort());
	    	if (!isAtReachable(commAtConnection)) {
	    		closeSerialPort(commAtConnection);
			...
			try {
				reply = commAtConnection.sendCommand(TelitGe865AtCommands.getRegistrationStatus.getCommand().getBytes(), 1000, 100);
			} catch (IOException e) {
				closeSerialPort(commAtConnection);
				throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
			}
    ...
    public long getCallTxCounter() throws KuraException {
    	...
    	long txCnt = 0;
    	synchronized (s_atLock) {
	    	s_logger.debug("sendCommand getGprsSessionDataVolume :: " + TelitGe865AtCommands.getGprsSessionDataVolume.getCommand());
	    	byte[] reply = null;
	    	CommConnection commAtConnection = openSerialPort(getAtPort());
	    	if (!isAtReachable(commAtConnection)) {
	    		closeSerialPort(commAtConnection);
			...
			try {
				reply = commAtConnection.sendCommand(TelitGe865AtCommands.getGprsSessionDataVolume.getCommand().getBytes(), 1000, 100);
			} catch (IOException e) {
				closeSerialPort(commAtConnection);
				throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
			}
    ...
    public String getServiceType() throws KuraException {
    	...
    	synchronized (s_atLock) {
    		s_logger.debug("sendCommand getMobileStationClass :: " + TelitGe865AtCommands.getMobileStationClass.getCommand());
	    	byte[] reply = null;
	    	CommConnection commAtConnection = openSerialPort(getAtPort());
	    	if (!isAtReachable(commAtConnection)) {
	    		closeSerialPort(commAtConnection);
			...
			try {
				reply = commAtConnection.sendCommand(TelitGe865AtCommands.getMobileStationClass.getCommand().getBytes(), 1000, 100);
			} catch (IOException e) {
				closeSerialPort(commAtConnection);
				throw new KuraException(KuraErrorCode.INTERNAL_ERROR, e);
			}
